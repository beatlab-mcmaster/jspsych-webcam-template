//Modified by Shreshth Saxena to take minimum height and width in standard units (mm) instead of pixel resolution. Only use after performing virtualchinrest calibration!
    ...
    class BrowserCheckPlugin {
        ...
        check_allow_resize(feature_data) {
            return __awaiter(this, void 0, void 0, function*() {
                ...
                var w = feature_data.get("width");
                var h = feature_data.get("height");
                ...
                if (this.t.allow_window_resize &&
                    ...
                    while (!this.end_flag &&
                        (window.innerWidth * px2mm < this.t.minimum_width || window.innerHeight * px2mm < this.t.minimum_height)) {
                        if (min_width_el) {
                        ...
                        if (actual_height_el) {
                            actual_height_el.innerHTML = (window.innerHeight * px2mm).toString();
                        }
                        if (actual_width_el) {
                            actual_width_el.innerHTML = (window.innerWidth * px2mm).toString();
                        }
                        yield this.delay(100);
                        feature_data.set("width", window.innerWidth * px2mm);
                        feature_data.set("height", window.innerHeight * px2mm);
                    }
                }
            });
        }
